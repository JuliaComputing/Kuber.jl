# This file was generated by the Julia Swagger Code Generator
# Do not modify this file directly. Modify the swagger specification instead.

type V1SELinuxOptions
    level::Nullable{ String } # level
    role::Nullable{ String } # role
    _type::Nullable{ String } # type
    user::Nullable{ String } # user

    function V1SELinuxOptions(;level=nothing, role=nothing, _type=nothing, user=nothing)
        o = new()
        set_field!(o, :level, level)
        set_field!(o, :role, role)
        set_field!(o, :_type, _type)
        set_field!(o, :user, user)
        o
    end
end # type V1SELinuxOptions

const _name_map_V1SELinuxOptions = Dict{String,Symbol}(["level"=>:level, "role"=>:role, "type"=>:_type, "user"=>:user])
Swagger.name_map(::Type{ V1SELinuxOptions }) = _name_map_V1SELinuxOptions

function check_required(o::V1SELinuxOptions)
    true
end

function set_field!(o::V1SELinuxOptions, name::Symbol, val)
    setfield!(o, name, fieldtype(V1SELinuxOptions,name)(val))
end

convert(::Type{ V1SELinuxOptions }, json::Dict{String,Any}) = Swagger.from_json(V1SELinuxOptions, json)
