# This file was generated by the Julia Swagger Code Generator
# Do not modify this file directly. Modify the swagger specification instead.

type VersionedEvent
    object::Nullable{ RuntimeRawExtension } # object
    _type::Nullable{ String } # type

    function VersionedEvent(;object=nothing, _type=nothing)
        o = new()
        set_field!(o, :object, object)
        set_field!(o, :_type, _type)
        o
    end
end # type VersionedEvent

const _name_map_VersionedEvent = Dict{String,Symbol}(["object"=>:object, "type"=>:_type])
Swagger.name_map(::Type{ VersionedEvent }) = _name_map_VersionedEvent

function check_required(o::VersionedEvent)
    isnull(o.object) && (return false)
    isnull(o._type) && (return false)
    true
end

function set_field!(o::VersionedEvent, name::Symbol, val)
    setfield!(o, name, fieldtype(VersionedEvent,name)(val))
end

convert(::Type{ VersionedEvent }, json::Dict{String,Any}) = Swagger.from_json(VersionedEvent, json)
