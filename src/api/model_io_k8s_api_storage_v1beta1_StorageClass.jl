# This file was generated by the Julia Swagger Code Generator
# Do not modify this file directly. Modify the swagger specification instead.

mutable struct IoK8sApiStorageV1beta1StorageClass <: SwaggerModel
    allowVolumeExpansion::Union{ Nothing, Bool } # allowVolumeExpansion
    allowedTopologies::Union{ Nothing, Vector{IoK8sApiCoreV1TopologySelectorTerm} } # allowedTopologies
    apiVersion::Union{ Nothing, String } # apiVersion
    kind::Union{ Nothing, String } # kind
    metadata::Union{ Nothing, IoK8sApimachineryPkgApisMetaV1ObjectMeta } # metadata
    mountOptions::Union{ Nothing, Vector{String} } # mountOptions
    parameters::Union{ Nothing, Dict{String, String} } # parameters
    provisioner::Union{ Nothing, String } # provisioner
    reclaimPolicy::Union{ Nothing, String } # reclaimPolicy
    volumeBindingMode::Union{ Nothing, String } # volumeBindingMode

    function IoK8sApiStorageV1beta1StorageClass(;allowVolumeExpansion=nothing, allowedTopologies=nothing, apiVersion=nothing, kind=nothing, metadata=nothing, mountOptions=nothing, parameters=nothing, provisioner=nothing, reclaimPolicy=nothing, volumeBindingMode=nothing)
        o = new()
        set_field!(o, :allowVolumeExpansion, allowVolumeExpansion)
        set_field!(o, :allowedTopologies, allowedTopologies)
        set_field!(o, :apiVersion, apiVersion)
        set_field!(o, :kind, kind)
        set_field!(o, :metadata, metadata)
        set_field!(o, :mountOptions, mountOptions)
        set_field!(o, :parameters, parameters)
        set_field!(o, :provisioner, provisioner)
        set_field!(o, :reclaimPolicy, reclaimPolicy)
        set_field!(o, :volumeBindingMode, volumeBindingMode)
        o
    end
end # type IoK8sApiStorageV1beta1StorageClass

const _name_map_IoK8sApiStorageV1beta1StorageClass = Dict{String,Symbol}(["allowVolumeExpansion"=>:allowVolumeExpansion, "allowedTopologies"=>:allowedTopologies, "apiVersion"=>:apiVersion, "kind"=>:kind, "metadata"=>:metadata, "mountOptions"=>:mountOptions, "parameters"=>:parameters, "provisioner"=>:provisioner, "reclaimPolicy"=>:reclaimPolicy, "volumeBindingMode"=>:volumeBindingMode])
const _field_map_IoK8sApiStorageV1beta1StorageClass = Dict{Symbol,String}([:allowVolumeExpansion=>"allowVolumeExpansion", :allowedTopologies=>"allowedTopologies", :apiVersion=>"apiVersion", :kind=>"kind", :metadata=>"metadata", :mountOptions=>"mountOptions", :parameters=>"parameters", :provisioner=>"provisioner", :reclaimPolicy=>"reclaimPolicy", :volumeBindingMode=>"volumeBindingMode"])
Swagger.name_map(::Type{ IoK8sApiStorageV1beta1StorageClass }) = _name_map_IoK8sApiStorageV1beta1StorageClass
Swagger.field_map(::Type{ IoK8sApiStorageV1beta1StorageClass }) = _field_map_IoK8sApiStorageV1beta1StorageClass

function check_required(o::IoK8sApiStorageV1beta1StorageClass)
    (o.provisioner === nothing) && (return false)
    true
end

function validate_field(o::IoK8sApiStorageV1beta1StorageClass, name::Symbol, val)
end
