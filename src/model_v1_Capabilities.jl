# This file was generated by the Julia Swagger Code Generator
# Do not modify this file directly. Modify the swagger specification instead.

type V1Capabilities
    add::Nullable{ Vector{String} } # add
    drop::Nullable{ Vector{String} } # drop

    function V1Capabilities(;add=nothing, drop=nothing)
        o = new()
        set_field!(o, :add, add)
        set_field!(o, :drop, drop)
        o
    end
end # type V1Capabilities

const _name_map_V1Capabilities = Dict{String,Symbol}(["add"=>:add, "drop"=>:drop])
Swagger.name_map(::Type{ V1Capabilities }) = _name_map_V1Capabilities

function check_required(o::V1Capabilities)
    true
end

function set_field!(o::V1Capabilities, name::Symbol, val)
    setfield!(o, name, fieldtype(V1Capabilities,name)(val))
end

convert(::Type{ V1Capabilities }, json::Dict{String,Any}) = Swagger.from_json(V1Capabilities, json)
