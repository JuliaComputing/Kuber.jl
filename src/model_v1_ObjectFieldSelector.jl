# This file was generated by the Julia Swagger Code Generator
# Do not modify this file directly. Modify the swagger specification instead.

type V1ObjectFieldSelector
    apiVersion::Nullable{ String } # apiVersion
    fieldPath::Nullable{ String } # fieldPath

    function V1ObjectFieldSelector(;apiVersion=nothing, fieldPath=nothing)
        o = new()
        set_field!(o, :apiVersion, apiVersion)
        set_field!(o, :fieldPath, fieldPath)
        o
    end
end # type V1ObjectFieldSelector

const _name_map_V1ObjectFieldSelector = Dict{String,Symbol}(["apiVersion"=>:apiVersion, "fieldPath"=>:fieldPath])
Swagger.name_map(::Type{ V1ObjectFieldSelector }) = _name_map_V1ObjectFieldSelector

function check_required(o::V1ObjectFieldSelector)
    isnull(o.fieldPath) && (return false)
    true
end

function set_field!(o::V1ObjectFieldSelector, name::Symbol, val)
    setfield!(o, name, fieldtype(V1ObjectFieldSelector,name)(val))
end

convert(::Type{ V1ObjectFieldSelector }, json::Dict{String,Any}) = Swagger.from_json(V1ObjectFieldSelector, json)
