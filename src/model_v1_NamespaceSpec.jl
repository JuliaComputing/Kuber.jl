# This file was generated by the Julia Swagger Code Generator
# Do not modify this file directly. Modify the swagger specification instead.

type V1NamespaceSpec
    finalizers::Nullable{ Vector{String} } # finalizers

    function V1NamespaceSpec(;finalizers=nothing)
        o = new()
        set_field!(o, :finalizers, finalizers)
        o
    end
end # type V1NamespaceSpec

const _name_map_V1NamespaceSpec = Dict{String,Symbol}(["finalizers"=>:finalizers])
Swagger.name_map(::Type{ V1NamespaceSpec }) = _name_map_V1NamespaceSpec

function check_required(o::V1NamespaceSpec)
    true
end

function set_field!(o::V1NamespaceSpec, name::Symbol, val)
    setfield!(o, name, fieldtype(V1NamespaceSpec,name)(val))
end

convert(::Type{ V1NamespaceSpec }, json::Dict{String,Any}) = Swagger.from_json(V1NamespaceSpec, json)
