# This file was generated by the Julia Swagger Code Generator
# Do not modify this file directly. Modify the swagger specification instead.

type V1alpha1RoleBinding
    metadata::Nullable{ V1ObjectMeta } # metadata
    roleRef::Nullable{ V1alpha1RoleRef } # roleRef
    subjects::Nullable{ Vector{V1alpha1Subject} } # subjects

    function V1alpha1RoleBinding(;metadata=nothing, roleRef=nothing, subjects=nothing)
        o = new()
        set_field!(o, :metadata, metadata)
        set_field!(o, :roleRef, roleRef)
        set_field!(o, :subjects, subjects)
        o
    end
end # type V1alpha1RoleBinding

const _name_map_V1alpha1RoleBinding = Dict{String,Symbol}(["metadata"=>:metadata, "roleRef"=>:roleRef, "subjects"=>:subjects])
Swagger.name_map(::Type{ V1alpha1RoleBinding }) = _name_map_V1alpha1RoleBinding

function check_required(o::V1alpha1RoleBinding)
    isnull(o.roleRef) && (return false)
    isnull(o.subjects) && (return false)
    true
end

function set_field!(o::V1alpha1RoleBinding, name::Symbol, val)
    setfield!(o, name, fieldtype(V1alpha1RoleBinding,name)(val))
end

convert(::Type{ V1alpha1RoleBinding }, json::Dict{String,Any}) = Swagger.from_json(V1alpha1RoleBinding, json)
